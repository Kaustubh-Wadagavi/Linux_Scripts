...
reportFileHtml=$(awk -F',' 'BEGIN { print "<table border=1 cellpadding=5 cellspacing=0>"} {print "<tr>"; for(i=1;i<=NF;i++) { if (NR==1) printf "<th><b><font color=\"blue\">%s</font></b></th>", $i; else printf "<td>%s</td>", $i } print "</tr>"} END{print "</table>"}' <(sed -E 's/^([^,]*,[^,]*,[^,]*,[^,]*,)("[^"]+",)(.*)/\1\2\3/; :a; s/("[^"]*),([^"]*")/\1\2/g; ta' "$inputDir/files/$fileName"))
...
getClientDetails() {
   ...
   do
     ...
     announcement=`echo ${getAnnouncement} | jq -r '.extensionDetail.attrs[].value'`
     currentVersion="v10.1.RC1"   #`echo ${getAnnouncement} | jq -r '.extensionDetail.attrs[].value'`
