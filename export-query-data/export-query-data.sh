...
exportQuery() {
  getDataFile=$(curl -H "X-OS-API-TOKEN: $token" -H "Content-Type: application/json" -X POST --data '{"drivingForm": "'"$drivingForm"'","cpId":-1,"aql": "'"$aql"'","wideRowMode": "'"$wideRowMode"'"}' "$url/rest/ng/query/export")
